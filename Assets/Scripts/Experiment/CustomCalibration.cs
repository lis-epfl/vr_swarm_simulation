//-----------------------------------------------------------------------
// Derived from the Tobii Unity SDK, see
// https://developer.tobiipro.com/unity/unity-sdk-reference-guide.html
//-----------------------------------------------------------------------
...
namespace Experiment
{
    ...
    {
        /// <summary>
        /// Instance of <see cref="CustomCalibration"/> for easy access.
        /// Assigned in Awake() so use earliest in Start().
        /// </summary>
        public static CustomCalibration Instance { get; private set; }
        ...
        // Custom countdown timer before calibration starts
        [SerializeField]
        ...
        public bool StartCalibration(Vector2[] points = null, System.Action<bool> resultCallback = null)
        {
            ...
            StartCoroutine(PerformCalibrationWithCountdown(points, resultCallback));
            return true;
        }

        private IEnumerator PerformCalibrationWithCountdown(Vector2[] points, System.Action<bool> resultCallback)
        {
            ...
            yield return StartCoroutine(PerformCalibration(points, resultCallback));
        }
        ...
        private IEnumerator PerformCalibration(Vector2[] points, System.Action<bool> resultCallback)
        {
            ...
            foreach (var pointPosition in _points)
            {
                ...
                if (collectResult.Status == Tobii.Research.CalibrationStatus.Failure)
                {
            ...
            LatestCalibrationSuccessful = computeResult.Status == Tobii.Research.CalibrationStatus.Success;
