...
| `maxAlpha` | 3.38 | rad/s^2 | Maximum angular acceleration, clamped about the world vertical / tilt split rather than about the body axes. **Rescaled by 0.3893 from the 8.68 the prefab used to hold**: the torque is now applied with `ForceMode.Force`, so this is the limit actually achieved, where before the inertia pre-multiply silently delivered 3.38 on pitch/roll and 6.67 on yaw. Same authority, honestly stated, and isotropic. |
...
| `headingHoldKp` | 8.0 | 1/s | The DJI flight controller's heading-hold gain, which the sim previously had no counterpart for. Closed-loop time constant ≈ `1/(headingHoldKp × 0.62)`, the 0.62 being the rate loop's DC gain against `angularDrag`. Useful range 6–12. |
| `maxHeadingHoldErrorDeg` | 25.0 | deg | Anti-windup on the integrated heading setpoint, the analogue of the fleet's `MAX_TARGET_LEAD_DEG` one level below `SwarmPlaneController.maxTargetLeadDeg`. Unconditional here because `headingHoldKp × 25°` (3.5 rad/s) is well above `maxYawRate` (1.309), so the rate clamp binds first and this costs no authority. |
