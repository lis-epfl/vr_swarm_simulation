...
class custom_stitcher_SP:
    ...
    def compute_matches_and_confidences(self, descriptors):
        ...
        for i in range(nb_img):
            ...
            for j in range(i + 1, nb_img):
                ...
                if num_matches> 4:
                    ...
                    # conf = np.sum(mask.ravel()) / (8 + 0.3 * num_matches)
    ...
    def compute_homographies_and_order(self, keypoints, matches_info, partial_order):
        ...
        for i in range(num_images):
            ...
            if dst_p.shape[0]>4:
                Hs[i], _ = cv2.findHomography(dst_p, src_p, method=cv2.RANSAC, ransacReprojThreshold=3, confidence=0.999)

        w, h = self.camera_matrix[:2, -1]
        
        corners_3d = np.array([[0, w-1 , w -1, 0],
                              [0, 0, h-1 , h-1 ],
                              [1, 1, 1, 1]], dtype=np.float32)
        new_corners_3d = np.dot(Hs[0], corners_3d)
        new_corners = new_corners_3d[:2]/new_corners_3d[-1]

        # mean_shift = np.mean(new_corners[0]-corners_3d[0])
        
        if new_corners[0,0]<0:
            Hs=Hs[::-1]
            order = partial_order[::-1]
    ...
    def second_thread(self, front_image_index=0, debug= False):
        ...
        while True:
            ...
            matches_info, confidences = self.compute_matches_and_confidences(descriptors)
    ...
    def compose_with_ref(self, images, Hs1, Hs2, subset1, subset2, inverted):
        ...
        # Create a larger panorama canvas if necessary
    ...
    def third_thread(self, angle=100, num_pano_img=3):
        ...
        while True:
            ...
            if odd:
                ...
                Hs1 = Hs[ref-offset:ref][::-1]
                
                subset2 = order[ref:ref+offset+1]
                Hs2 = Hs[ref:ref+offset]
...
def read_images_from_shared_memory(batchMMF, batchFlagPosition, imageCount, batchDataPosition, imageSize, imageHeight, imageWidth):
    while True:
        ...
        flag = struct.unpack('i', batchMMF.read(4))[0]
        ...
        if flag == 0:  # Unity isn't writing new images
            ...
            batchMMF.seek(batchFlagPosition)
            batchMMF.write(struct.pack('i', 1))
            ...
            batchMMF.seek(batchFlagPosition)
            batchMMF.write(struct.pack('i', 0))
